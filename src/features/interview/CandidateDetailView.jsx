...
export default function CandidateDetailView({ candidate }) {
  ...
  const exportJSON = () => {
    ...
    a.href = url;
    a.download = `candidate-${candidate.name || candidate._id}.json`;
    a.click();
  ...
  const exportPDF = () => {
    ...
    doc.save(`candidate-${candidate.name || candidate._id}.pdf`);
  };
...
CandidateDetailView.defaultProps = {
  candidate: null,
};
