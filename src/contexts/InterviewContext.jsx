...
const interviewReducer = (state, action) => {
  switch (action.type) {
    ...
    case ACTIONS.START_INTERVIEW_OPTIMISTIC: {
      ...
      const { name, email, phone, role } = action.payload;
...
export function InterviewProvider({ children }) {
  ...
  const startInterview = useCallback(async ({ name, email, phone, role, resume }) => {
  ...
  const submitAnswer = useCallback(async ({ sessionId, answerText }, optimistic = true) => {
    // Dispatch optimistic update immediately
    if (optimistic) {
      dispatch({
