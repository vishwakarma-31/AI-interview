...
const { getDatabaseCredentials, getJWTSecrets, getOpenAIApiKey } = require('./utils/secretsManager');
...
async function initializeSecrets() {
  try {
    ...
    process.env.MONGO_URI = decryptEnvVar(dbCredentials.uri || process.env.MONGO_URI || config.mongo.uri);
    
    // Get JWT secrets
    const jwtSecrets = await getJWTSecrets();
    process.env.JWT_SECRET = decryptEnvVar(jwtSecrets.accessTokenSecret || process.env.JWT_SECRET);
    process.env.JWT_REFRESH_SECRET = decryptEnvVar(jwtSecrets.refreshTokenSecret || process.env.JWT_REFRESH_SECRET);
    
    // Get OpenAI API key
    const openaiApiKey = await getOpenAIApiKey();
    process.env.OPENAI_API_KEY = decryptEnvVar(openaiApiKey || process.env.OPENAI_API_KEY);
    
    // Get other secrets
    process.env.SESSION_SECRET = decryptEnvVar(process.env.SESSION_SECRET || config.session.secret);
    process.env.ENCRYPTION_KEY = decryptEnvVar(process.env.ENCRYPTION_KEY);
    process.env.SENTRY_DSN = decryptEnvVar(process.env.SENTRY_DSN);
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      scriptSrc: ["'self'", "'strict-dynamic'"], // Enhanced script security
...
app.get('/health', (req, res) => {
  ...
  res.status(200).json(formatSuccessResponse({ status: 'OK' }, 'AI Interview Assistant Backend is running'));
});
